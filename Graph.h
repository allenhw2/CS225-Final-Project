...
/**
 * Class adapted from CS225 lab ML
 */
class Graph
{
public:
    Graph(vector<int> weights, vector<Vertex> fromNode, vector<Vertex> toNode);

    /**
     * Default constructor
     * Required for BFS to work
     */
    Graph() {
        //nothing
    }
    ...
    pair<int, vector<Vertex>> aStarPath(Vertex start, Vertex end);
    pair<int, vector<Vertex>> dijkstrasPath(Vertex start, Vertex end);
